...
fn test_derive_keys_version_d_aes_128() {
    ...
    let (kbek, kbak) =
        derive_keys_version_d(&provider, kbpk.as_slice(), AesKeySize::Bits128).unwrap();
...
fn test_derive_keys_version_d_aes_192() {
    ...
    let (kbek, kbak) =
        derive_keys_version_d(&provider, kbpk.as_slice(), AesKeySize::Bits192).unwrap();
...
fn test_derive_keys_version_d_aes_256() {
    ...
    let (kbek, kbak) =
        derive_keys_version_d(&provider, kbpk.as_slice(), AesKeySize::Bits256).unwrap();
